...
media:	.int -1								#En este caso, debido a la naturaleza de la instrucción idiv, usamos dos
resto:	.int -1 							#variables enteras, una guardará el cociente de la división y otra el resto.
...
_start:	.global _start							#Sección donde empieza el código, equiparable al main() en C.
	...
	idiv %ecx							#Calculada la suma, guardada en EDX:EAX, la instrucción idiv calcula la
									#división EAX:EDX entre el valor dado por argumento, en este caso la longitud
									#de la lista, guardada en %ecx, y guarda el cociente en %eax y el resto en
									#edx.
	mov %eax, media							#Por lo descrito anteriormente, guardamos en la variable media el contenido de
	mov %edx, resto							#%eax y en resto el contenido de %edx.
